...
struct Prediction{
	...
	std::unique_ptr<std::vector<std::vector<float>>> kps_scores;
	std::unique_ptr<std::vector<std::vector<float>>> keypoints;
};
...
void ModelLoader::make_prediction(std::vector<Tensor> &image_output, Prediction &out_pred){
	...
	std::vector<string> output_nodes = {{
				"StatefulPartitionedCall:0", //detection_boxes
				"StatefulPartitionedCall:1", //detection_boxes_strided
				"StatefulPartitionedCall:2", //detection_classes
				"StatefulPartitionedCall:3", //detection_keypoints_scores
				"StatefulPartitionedCall:4", //detection_keypoints         
				"StatefulPartitionedCall:5", //detection_multiclass_scores
				"StatefulPartitionedCall:6", //detection_scores                
				"StatefulPartitionedCall:7", //num_detections
	}};
	...
	auto predicted_boxes = predictions[0].tensor<float, 3>();
	auto predicted_scores = predictions[6].tensor<float, 2>();
	auto predicted_labels = predictions[2].tensor<float, 2>();
	auto predicted_kps_scores = predictions[3].tensor<float, 3>();
	auto predicted_kp = predictions[4].tensor<float, 4>();
	
	//inflate with predictions
	for (int i=0; i < 100; i++){
		std::vector<float> coords;
		std::vector<float> kp_coords;
		std::vector<float> kp_scores_coords;
		for (int j=0; j <4 ; j++){
			coords.push_back( predicted_boxes(0, i, j));
		} 
		for (int j=0; j <6 ; j++){
			kp_scores_coords.push_back( predicted_kps_scores(0, i, j));
			kp_coords.push_back( predicted_kp(0, i, j, 0));
			kp_coords.push_back( predicted_kp(0, i, j, 1));
		}
		...
		(*out_pred.kps_scores).push_back(kp_scores_coords);
		(*out_pred.keypoints).push_back(kp_coords);
	}
}
