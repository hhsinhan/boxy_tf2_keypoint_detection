  ...
  with tf.name_scope('RandomHorizontalFlip', values=[image, boxes]):
    ...
    if keypoints is not None and keypoint_flip_permutation is not None:
      ...
      keypoints = tf.cond(
          ...
          lambda: keypoints)
      # boxy: switch y of boxy lt, rb
      def _change_lt_rb(keypoints):
          keypoints_lt_rb = keypoints[:,:2,:]
          y = tf.reverse(keypoints_lt_rb[:, :, 0], [1])
          x = keypoints_lt_rb[:, :, 1]
          keypoints_lt_rb = tf.concat([tf.expand_dims(y, -1), tf.expand_dims(x, -1)], 2)
          return tf.concat([keypoints_lt_rb, keypoints[:,2:,:]], 1)

      keypoints = tf.cond(
          do_a_flip_random,
          lambda: _change_lt_rb(keypoints),
          lambda: keypoints)
...
def remove_small_objects(
                      ...
                      densepose_part_ids=None,
                      densepose_surface_coords=None,
                      largest_output_feature_width=300,
                      largest_output_feature_height=300,
                      seed=None,
                      preprocess_vars_cache=None):
  # 0 ymin
  # 1 xmin
  # 2 ymax
  # 3 xmax
  objs_width = (boxes[:, 3] - boxes[:, 1]) * largest_output_feature_width
  objs_height = (boxes[:, 2] - boxes[:, 0]) * largest_output_feature_height

  objs_mask = tf.logical_and(
      tf.greater_equal(objs_width, tf.constant(1.0)),
      tf.greater_equal(objs_height, tf.constant(1.0))
  )

  objs_mask_idx = tf.where(objs_mask)[:,0]

  boxes = tf.gather(boxes,objs_mask_idx)
  labels = tf.gather(labels,objs_mask_idx)
  outputs = [image,boxes,labels]

  if label_weights is not None:
    label_weights = tf.gather(label_weights,objs_mask_idx)
    outputs.append(label_weights)
  if label_confidences is not None:
    label_confidences = tf.gather(label_confidences, objs_mask_idx)
  ...
  if keypoints is not None:
    keypoints = tf.gather(keypoints, objs_mask_idx)
    outputs.append(keypoints)
  if keypoint_visibilities is not None:
    keypoint_visibilities = tf.gather(keypoint_visibilities, objs_mask_idx)
